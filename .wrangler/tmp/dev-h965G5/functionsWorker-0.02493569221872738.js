...
// .wrangler/tmp/pages-AwNjWm/functionsWorker-0.02493569221872738.mjs
...
var app = new Hono2();
var FIRESTORE_API_KEY = "AIzaSyAiO44kikucTKZgRSzoEv-NM2WNqAXBkGk";
var FIRESTORE_PROJECT_ID = "codeshare-3e48f";
var FIRESTORE_BASE_URL = `https://firestore.googleapis.com/v1/projects/${FIRESTORE_PROJECT_ID}/databases/(default)/documents`;
var DEFAULT_JWT_SECRET = "devdrop_default_jwt_secret_change_me_in_production";
async function getCryptoKey(secret, mode) {
...
async function verifySession(signedData, secret) {
...
__name(mapFirestoreDoc, "mapFirestoreDoc");
__name2(mapFirestoreDoc, "mapFirestoreDoc");
...
__name2(toFirestoreFields, "toFirestoreFields");
...
__name(updateWorkspaceTimestamp, "updateWorkspaceTimestamp");
__name2(updateWorkspaceTimestamp, "updateWorkspaceTimestamp");
app.use("*", cors({
  origin: /* @__PURE__ */ __name2((origin) => {
    if (!origin) return "https://devdrop.harshtiwari.dev";
    if (origin.startsWith("http://localhost:") || origin.endsWith(".harshtiwari.dev") || origin === "https://devdrop.harshtiwari.dev") {
      return origin;
    }
...
async function authMiddleware(c, next) {
  ...
  const match3 = cookies.match(/devdrop_session=([^;]+)/);
  const sessionVal = match3 ? decodeURIComponent(match3[1]) : null;
...
app.post("/api/login", async (c) => {
  ...
  const actualPassword = c.env.DEVDROP_PASSWORD || "harsh@78";
...
app.get("/api/workspace/status", async (c) => {
  ...
  try {
    ...
    if (!res.ok) {
      return c.json({ updatedAt: (/* @__PURE__ */ new Date(0)).toISOString() });
    }
    const data = await res.json();
    const mapped = mapFirestoreDoc(data);
    return c.json({ updatedAt: mapped.updatedAt || (/* @__PURE__ */ new Date(0)).toISOString() });
  } catch (e) {
...
app.delete("/api/workspace/:id", async (c) => {
  ...
  try {
    ...
    if (res.ok) {
      const data = await res.json();
      if (data.documents) {
        const deletePromises = data.documents.map(async (doc) => {
...
app.post("/api/file", async (c) => {
  const body = await c.req.json().catch(() => ({}));
  const { name, content, language, order } = body;
  if (!name) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return c.json({ error: "Failed to create file in Firestore", details: errText }, 500);
    }
    await updateWorkspaceTimestamp();
...
app.put("/api/file/:id", async (c) => {
  ...
  if (name !== void 0) payload.name = name.trim();
  if (content !== void 0) payload.content = content;
  if (language !== void 0) payload.language = language;
  if (order !== void 0) payload.order = order;
  ...
  try {
    ...
    if (!res.ok) {
      const errText = await res.text();
      return c.json({ error: "Failed to update file", details: errText }, 500);
    }
    await updateWorkspaceTimestamp();
    return c.json({ success: true });
  } catch (e) {
...
app.delete("/api/file/:id", async (c) => {
  ...
  try {
    ...
    if (!res.ok) {
      const errText = await res.text();
      return c.json({ error: "Failed to delete file", details: errText }, 500);
    }
    await updateWorkspaceTimestamp();
    return c.json({ success: true });
  } catch (e) {
...
app.post("/api/duplicate/:id", async (c) => {
  ...
  try {
    ...
    if (!getRes.ok) {
      return c.json({ error: "File to duplicate not found" }, 404);
    }
    const doc = await getRes.json();
    const file = mapFirestoreDoc(doc);
    const newFileId = Math.random().toString(36).substring(2, 15);
    const postUrl = `${FIRESTORE_BASE_URL}/workspace/default/files/${newFileId}?key=${FIRESTORE_API_KEY}`;
    ...
    if (!createRes.ok) {
      return c.json({ error: "Failed to create duplicated file" }, 500);
    }
    await updateWorkspaceTimestamp();
    return c.json(payload);
  } catch (e) {
...
// .wrangler/tmp/bundle-T5VXfw/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-T5VXfw/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.02493569221872738.js.map
